...
impl Editor {
    ...
    pub fn key_context(&self, window: &mut Window, cx: &mut App) -> KeyContext {
        ...
        // Disable vim contexts when a nested editor is focused.
    ...
    fn render_gutter_hover_button(
        ...
        if !show_bookmarks {
            log::error!("Trying to place gutter_hover without bookmarks enabled");
        }
    ...
    pub fn refresh_inline_values(&mut self, cx: &mut Context<Self>) {
        ...
        self.inline_value_cache.refresh_task = Task::ready(Some(()));
    }
