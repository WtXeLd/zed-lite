...
use zed_actions::{About, OpenBrowser, OpenDocs, OpenSettingsFile, OpenZedUrl, Quit};
...
fn open_about_window(cx: &mut App) {
    fn about_window_icon() -> Arc<Image> {
        let bytes = include_bytes!("../resources/app-icon.png").as_slice();
...
pub fn load_default_keymap(cx: &mut App) {
    ...
    let default_keymap = KeymapFile::load_asset_allow_partial_failure(DEFAULT_KEYMAP_PATH, cx).map(
        |mut key_bindings| {
    ...
    cx.bind_keys(default_keymap.unwrap());

    if let Some(asset_path) = base_keymap.asset_path() {
        let base_keymap =
            KeymapFile::load_asset_allow_partial_failure(asset_path, cx).map(|mut key_bindings| {
                for key_binding in &mut key_bindings {
                    key_binding.set_meta(KeybindSource::Base.meta());
                }
                key_bindings
            });
        cx.bind_keys(base_keymap.unwrap());
    }

    if VimModeSetting::get_global(cx).0 || vim_mode_setting::HelixModeSetting::get_global(cx).0 {
        let vim_keymap = KeymapFile::load_asset_allow_partial_failure(VIM_KEYMAP_PATH, cx).map(
            |mut key_bindings| {
                for key_binding in &mut key_bindings {
        ...
        cx.bind_keys(vim_keymap.unwrap());
    }
}
        ...
        workspace
            .update_in(cx, |workspace, window, cx| {
                ...
                cx.spawn_in(window, async move |workspace, cx| {
                    ...
                    // Set up a dedicated worktree for settings, since otherwise we're
                    // dropping and re-starting LSP servers for each file inside on every
                    // settings file close/open.
